...
static struct ntfs_device_operations *mkntfs_device_ops_override = NULL;
static void *mkntfs_device_private_override = NULL;

void mkntfs_set_device_override(struct ntfs_device_operations *ops, void *private_data)
{
	mkntfs_device_ops_override = ops;
	mkntfs_device_private_override = private_data;
}

void mkntfs_clear_device_override(void)
{
	mkntfs_device_ops_override = NULL;
	mkntfs_device_private_override = NULL;
}
...
static BOOL mkntfs_open_partition(ntfs_volume *vol)
{
	...
	struct ntfs_device_operations *dev_ops = mkntfs_device_ops_override;
	void *dev_private = mkntfs_device_private_override;

	if (!dev_ops) {
#ifdef NO_NTFS_DEVICE_DEFAULT_IO_OPS
		ntfs_log_error("No default device io operations are available.\n");
		goto done;
#else
		dev_ops = &ntfs_device_default_io_ops;
#endif
	}
	 ...
	 * the volume.
	 */
	vol->dev = ntfs_device_alloc(opts.dev_name, 0, dev_ops, dev_private);
	if (!vol->dev) {
