...
#else
#include <wchar.h>
static void StringCchCopyW (wchar_t *dst, size_t dstLen, const wchar_t *src)
{
	if (dstLen == 0) return;
	wcsncpy (dst, src, dstLen);
	dst[dstLen - 1] = 0;
}
static void StringCchCatW (wchar_t *dst, size_t dstLen, const wchar_t *src)
{
	size_t len = wcslen (dst);
	if (len >= dstLen) return;
	wcsncpy (dst + len, src, dstLen - len);
	dst[dstLen - 1] = 0;
}
#endif
#endif
