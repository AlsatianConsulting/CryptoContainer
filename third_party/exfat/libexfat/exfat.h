...
struct exfat_dev;
struct exfat_dev_ops
{
	ssize_t (*pread)(void* user, void* buffer, size_t size, off_t offset);
	ssize_t (*pwrite)(void* user, const void* buffer, size_t size, off_t offset);
	int (*fsync)(void* user);
	int (*close)(void* user);
};
struct exfat_dev_ops;
...
int exfat_mount_dev(struct exfat* ef, struct exfat_dev* dev, const char* options);
