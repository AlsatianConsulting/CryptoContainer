...
struct exfat_dev* exfat_open(const char* spec, enum exfat_mode mode)
{
	...
	memset(&dev->ops, 0, sizeof(dev->ops));
	dev->user = NULL;
	dev->pos = 0;
...
struct exfat_dev* exfat_open_ops(const struct exfat_dev_ops* ops, void* user,
		enum exfat_mode mode, off_t size)
{
	struct exfat_dev* dev;

	if (!ops || !ops->pread)
	{
		exfat_error("failed to open device: missing pread callback");
		return NULL;
	}
	...
	{
		free(dev);
		exfat_error("failed to open device: invalid size");
		return NULL;
	}
...
ssize_t exfat_read(struct exfat_dev* dev, void* buffer, size_t size)
{
	if (dev->ops.pread)
	{
		ssize_t result = dev->ops.pread(dev->user, buffer, size, dev->pos);
		if (result >= 0)
			dev->pos += result;
		return result;
	}
...
ssize_t exfat_write(struct exfat_dev* dev, const void* buffer, size_t size)
{
	if (dev->ops.pwrite)
	{
		ssize_t result = dev->ops.pwrite(dev->user, buffer, size, dev->pos);
		if (result >= 0)
			dev->pos += result;
		return result;
	}
...
{
	if (dev->ops.pread)
		return dev->ops.pread(dev->user, buffer, size, offset);
#ifdef USE_UBLIO
...
{
	if (dev->ops.pwrite)
		return dev->ops.pwrite(dev->user, buffer, size, offset);
#ifdef USE_UBLIO
