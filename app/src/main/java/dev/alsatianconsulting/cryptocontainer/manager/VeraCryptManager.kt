...
class VeraCryptManager {
    ...
    /**
     * Creates a new VeraCrypt volume using the settings in [options].
     *
     * **Ownership note:** [VolumeCreateOptions.password] and [VolumeCreateOptions.hiddenPassword]
     * are zeroed (filled with `'\u0000'`) before this function returns, regardless of success or
     * failure.  Callers must not reuse these arrays after the call.
     */
    suspend fun create(context: Context, options: VolumeCreateOptions): Int {
        return withContext(Dispatchers.IO) {
            lock.withLock {
                try {
                    val passwordBytes = charArrayToUtf8Bytes(options.password)
                    val hiddenPasswordBytes = options.hiddenPassword?.let { charArrayToUtf8Bytes(it) } ?: ByteArray(0)
                    ...
                    try {
                        ...
                        options.password.fill('\u0000')
                        options.hiddenPassword?.fill('\u0000')
                    }
                } catch (t: Throwable) {
